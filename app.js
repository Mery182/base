...
function reqPost(request, response) {
	if (request.method == 'POST') {
		...
		request.on('end', function () {
			...
			// var sInsert = "INSERT INTO loans (text, description, keywords) VALUES (\"" + post['col1'] + "\",\"" + post['col2'] + "\",\"" + post['col3'] + "\")";
			var sInsert = "INSERT INTO loans (text, description, keywords) VALUES (\"" + post['col1'] + "\",\"" + post['col2'] + "\",\"" + post['col3'] + post['col4'] + "\",\"" + post['col5'] + "\")";
...
function ViewSelect(res) {
	...
	var results = connection.query('SELECT * FROM loans WHERE id>14 ORDER BY id DESC');
	for (let i = 0; i < results.length; i++)
